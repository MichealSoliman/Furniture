...
// // لتحديد حجم الصور بشكل عشوائي (صغير/كبير)
// const getRandomSpan = () => {
//   const colSpan = Math.random() > 0.7 ? "col-span-2" : "col-span-1";
//   const rowSpan = Math.random() > 0.6 ? "row-span-2" : "row-span-1";
...
//       {/* Gallery Grid */}
//       <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 gap-2 auto-rows-[150px]">
//         {galleryData[activeTab].map((imgSrc, index) => (
//           <div
//             key={index}
//             className={`${getRandomSpan()} overflow-hidden rounded-lg`}
//           >
//             <img
//               src={imgSrc}
//               alt={`صورة أثاث ${index + 1}`}
    ...
    <section className="py-16 px-6 text-center" dir="rtl" id="gallery">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 gap-2 auto-rows-[150px]">
        {galleryData[activeTab].map((imgSrc, index) => (
          <div
            key={index}
            className={`${getRandomSpan()} overflow-hidden rounded-lg animate-fade-in`}
            style={{
